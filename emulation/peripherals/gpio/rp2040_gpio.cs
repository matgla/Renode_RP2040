...
namespace Antmicro.Renode.Peripherals.GPIOPort
{
    ...
    public class RP2040GPIO : BaseGPIOPort, IRP2040Peripheral, IDoubleWordPeripheral, IGPIOReceiver, IKnownSize
    {
        ...
        private void SetPinAccordingToPulls(int pin)
        {
            ...
            {
                State[pin] = false;
                UpdateStateBitmap(pin, false);
                Connections[pin].Set(false);
            }
            if (pullUp[pin] == true)
            {
                State[pin] = true;
                UpdateStateBitmap(pin, true);
        ...
        public void WritePin(int number, bool value, GpioFunction peri, bool forced = false)
        {
            ...
            State[number] = value;
            UpdateStateBitmap(number, value);
        ...
        private void UpdateStateBitmap(int pin, bool value)
        {
            var bit = 1u << pin;
            if (value)
            {
